...
class HandCodedLaneFollower(object):
    ...
    def steer(self, frame, lane_lines):
        ...
        if self.car is not None:
            picarx_steering_angle = self.curr_steering_angle - 90    #picar-x straight = 90
            #self.car.front_wheels.turn(self.curr_steering_angle)
            self.car.set_dir_servo_angle(picarx_steering_angle)
...
def detect_edges(frame):
    ...
    #picar-x to detect black lane strips, using same variables
...
def detect_line_segments(cropped_edges):
    ...
    min_threshold = 7  # minimal of votes
...
def show_image(title, frame, show=_SHOW_IMAGE):
    path = '/home/pi/picar-x/tests/tmp_res/'
    if show:
        #cv2.imshow(title, frame)
        cv2.imwrite(f'{path}{title+".png"}', frame) # saving image in local storage
...
if __name__ == '__main__':
    ...
    test_photo('/home/pi/picar-x/tests/tmp/test_image.png')
